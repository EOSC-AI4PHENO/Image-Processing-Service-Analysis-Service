...
public class Get90PercentileReport extends AnalysisReportAbstract{

    @Getter
    public List<String> parameterNames = List.of("r_av","g_av","b_av", "bri_av", "gi_av", "bi_av", "ri_av","number_of_lindens","isFlowering");
    public Get90PercentileReport(Long analysisId, LocalDate dateFrom, LocalDate dateTo, String sort) {
    ...
    private String sql =
            "select * from crosstab (" +
                    "   'SELECT     " +
                    "                dense_rank() OVER (ORDER BY ad.\"analysisId\", da.\"day\")::int AS row_name,  " +
                    "                ad.\"analysisId\", " +
                    "                da.\"day\" AS \"date\", " +
                    "                ar.\"parameterId\", " +
                    "                PERCENTILE_CONT(0.9) WITHIN GROUP(ORDER BY ar.value) AS value " +
                    "            FROM public.\"AnalysisResult\" ar " +
                    "                INNER JOIN public.\"AnalysisDetail\" ad " +
                    "                   ON ar.\"detailsId\"  = ad.id " +
                    "                INNER JOIN public.generateWindowsDates(''__dateFrom__'', ''__dateTo__'') da " +
                    "                   ON da.dateFrom <= CAST(ad.\"imageCreatedAt\" AS DATE) AND " +
                    "                      da.dateTo >= CAST(ad.\"imageCreatedAt\" AS DATE)" +
                    "            WHERE " +
                    "                ad.\"analysisId\"  = __analysisId__ AND " +
                    "                ar.\"isActive\" = TRUE AND ad.\"isActive\" = TRUE AND " +
                    "                ar.\"parameterId\" IN (__ids__) "  +
                    "            GROUP BY " +
                    "                ad.\"analysisId\", ar.\"parameterId\", da.\"day\"', " +
