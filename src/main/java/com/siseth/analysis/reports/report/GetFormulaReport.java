...
public class GetFormulaReport extends AnalysisReportAbstract{
    ...
    public GetFormulaReport(Long analysisId, LocalDate dateFrom, LocalDate dateTo, String sort, Pattern pattern) {
        super(analysisId, dateFrom, dateTo, true, sort);
        this.parameterNames = extractStrings(pattern.getFormula());
    }

    public static List<String> extractStrings(String input) {
        List<String> stringsBetweenBraces = new ArrayList<>();

        java.util.regex.Pattern pattern = java.util.regex.Pattern.compile("\\{(.*?)\\}");
        Matcher matcher = pattern.matcher(input);

        while (matcher.find()) {
            stringsBetweenBraces.add(matcher.group(1));
        }

        return stringsBetweenBraces;
    }
